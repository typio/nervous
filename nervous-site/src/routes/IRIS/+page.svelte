  ...
  if (browser) {
    (async () => {
      ...
      main = async () => {
        ...
        init_train_acc =
          (await evaluate(trainDataTensor, trainLabelsTensor, W, b))[1] + "%";
        final_train_acc = "";

        init_test_acc =
          (await evaluate(testDataTensor, testLabelsTensor, W, b))[1] + "%";
        final_test_acc = "";
        ...
        const fit = async () => {
          ...
          let data_loss = correct_logprobs.sum().values() / train_samples_n;
          ...
          let dW = await trainDataTensor.transpose().matmul(dOutput);
      ...
      p.draw = () => {
        ...
        for (let i = 0; i < 4; i++) {
          ...
          for (let j = 0; j < 3; j++) {
            ...
            let midY =
              (1 - t) * (50 + i * 100) + t * (100 + j * 100) - y_offset;
...
<body class="text-slate-700 ">
  ...
  <div class="max-w-3xl mx-auto mb-12">
    ...
    <div class="mt-8 indent-8 space-y-4">
      <p>
        This webpage trains a neural network, specifically a linear classifier,
        within your web browser to predict the species of an Iris flower based
        on its input variables. The Iris flower dataset contains the input
        variables (features) of Sepal Length, Sepal Width, Petal Length, and
        Petal Width for 150 different Iris flowers. These flowers belong to
      ...
      <p>
        ...
        by a matrix of weights <Katex math="(W)" />, and a vector <Katex
          math="(b)"
        /> is added to the product. This produces a set of weighted input values
        that represent the relative importance of each input variable in predicting
        the species of the flower. The index of the largest value in the resultant
        vector is the predicted species of the flower <Katex math="(y)" />.
        Initially, <Katex math="W" /> and <Katex math="b" /> are assigned random
        values, but the neural network learns by adjusting these values in a way
        that minimizes future error.
      </p>
      <p>
        Error is calculated using the softmax and cross-entropy functions. The
        softmax function assigns a probability to each possible output value
        (i.e. each species of Iris flower), and the cross-entropy function
        measures the difference between the predicted probabilities and the true
        probabilities (i.e. the actual species of the flower). By minimizing
        this difference, the neural network improves its accuracy in correctly
        predicting the species of an Iris flower from its input variables.
      </p>
