...
export const matmul = (a: Tensor, m: Tensor) => {
    ...
    if (
        (a.rank() === 1 && m.rank() === 1) ||
        (a.rank() === 2 && m.rank() === 2 && a.shape()[0] === 1 && m.shape()[1] === 1)
    ) {
        let newV: number = 0
        for (let i = 0; i < a.flatValues().length; i++) newV += a.values()[i] * m.values()[i]
    ...
    if (a.rank() === 2 && m.rank() === 2) {
        if (a.shape()[1] !== m.shape()[0]) throw new Error("Tensors doesn't have compatible shapes for multiplication.")
        ...
        for (let i = 0; i < newV.length; i++) {
            ...
            for (let j = 0; j < newV[i].length; j++) newV[i][j] = 0
        }
